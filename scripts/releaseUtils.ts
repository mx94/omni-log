...
export function checkNpmRegistry() {
  return true
}

export const packages = ['log-mini', 'log-web']
...
export function readPackage(pkgPath: string) {
...
export async function publishPkg(tag: string) {
  let pkgName = ''
  let version
  ...
  if (!checkNpmRegistry()) {
    throw new Error(`发布前请检查 npm registry 配置`)
  }
